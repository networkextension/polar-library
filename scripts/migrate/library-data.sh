#!/usr/bin/env bash
# library-data.sh — copy rev_* tables from ideamesh → polar_library.
set -euo pipefail

SRC_DSN="${SRC_DSN:-postgres://ideamesh:test123456@127.0.0.1:5432/ideamesh}"
DST_DSN="${DST_DSN:-postgres://ideamesh:test123456@127.0.0.1:5432/polar_library}"
...
TABLES=(rev_devices rev_firmwares rev_functions)

echo "=== library-data.sh — $(if [[ $APPLY -eq 1 ]]; then echo APPLY; else echo DRY-RUN; fi) ==="
echo "source: $SRC_DSN"
echo "target: $DST_DSN"
echo
...
TMPDIR=$(mktemp -d -t libmigrate)
trap 'rm -rf "$TMPDIR"' EXIT
DUMP="$TMPDIR/library-data.sql"
