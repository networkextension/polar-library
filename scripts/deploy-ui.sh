#!/usr/bin/env bash
# Build the library plugin UI and rsync it to .57.
#
# Pairs with the nginx vhost change that points `library.4950.store` at
# `/Users/local/var/polar-ui/library/`. Cross-page sidebar links (e.g.
# /dashboard.html) fall back to dock's /ui/dist via nginx try_files, so
# this dir only needs to carry library's own files.
#
# Required env:
...
#   DEPLOY_PATH   default: /Users/local/var/polar-ui/library
#
# Usage from the polar-library repo root:
...
DEPLOY_PATH="${DEPLOY_PATH:-/Users/local/var/polar-ui/library}"
