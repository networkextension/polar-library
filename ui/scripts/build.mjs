// Build the library plugin UI.
//
//   src/library.ts  -> dist/scripts/library.js   (esbuild bundle, ESM)
...
await esbuild.build({
    entryPoints: [path.join(srcDir, "library.ts")],
    bundle: true,
    format: "esm",
    target: ["es2022"],
    platform: "browser",
...
// Copy public/ assets verbatim — library.html and anything else the
...
console.log("built library plugin UI -> dist/");
